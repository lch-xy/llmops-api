...
from internal.core.tools.builtin_tools.entites.tool_entity import ToolEntity
...
class Provider(BaseModel):
    ...
    def __init__(self, **kwargs):
        """当我们实例化 Provider(name='google') 时，自动触发初始化"""
        super().__init__(**kwargs)
        self._provider_init()  # 实例化时自动触发

    def get_tool(self, tool_name: str) -> Any:
        return self.tool_func_map.get(tool_name)

    def get_tool_entity(self) -> list[ToolEntity]:
        return list(self.tool_entity_map.values())

    def _provider_init(self):
        """自动从 YAML 文件里读取配置，填满百宝箱和武器库"""
        # 1. 顺藤摸瓜，找到当前厂商配置所在的绝对路径
        ...
        # 2. 读取该厂商的 positions.yaml，看它名下到底有几个工具
        ...
        for tool_name in positions_yaml_data:
            # 读取对应的工具配置文件 (例如 google_search.yaml)
            ...
            # 把读出来的 YAML 字典，转换成我们上一节写的 ToolEntity，丢进百宝箱。
            # 在字典前面加两个星号 **，叫做字典解包。
            self.tool_entity_map[tool_name] = ToolEntity(**tool_yaml_data)
            # 【使用扳手】动态去抓取同名 python 文件里的函数，丢进武器库。主要是将代码转变成配置了，方便维护
